...
interface FullscreenScreenProps {
  isOpen: boolean;
  onClose: () => void;
  title: string;
  subtitle?: string;
...
export function FullscreenScreen({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-3">
          ...
          <div className="flex flex-col items-center">
            {subtitle && (
              <span className="text-xs tracking-[0.2em] text-[#a87a5d] font-serif">{subtitle}</span>
            )}
            <span className="font-serif text-lg text-[#f5e6b8] tracking-wider">{title}</span>
          </div>
        </div>
      ...
      {/* 主内容区 - 全屏沉浸 */}
      <main className="relative flex-1 min-h-0 overflow-hidden p-4 md:p-6">
        {children}
      </main>
