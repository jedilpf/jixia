...
const CARD_TYPES: { type: CardTypeV2 | 'all'; label: string; color: string; icon: string; bg: string }[] = [
  { type: 'all', label: '万象', color: '#1A1A1A', icon: '象', bg: '#F5F5F5' },
              ...
              <div className="flex items-center gap-6 mt-2">
                ...
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[11px] font-black text-[#1A1A1A]/40 uppercase tracking-widest">
                     策论定额: 2★≤{currentQuota.maxTwoStar} / 3★≤{currentQuota.maxThreeStar}
                   </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 p-10 overflow-y-auto scrollbar-hide bg-gradient-to-r from-transparent to-[#1A1A1A]/5">
            {filteredCards.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center opacity-10">
                ...
                <p className="text-2xl font-black tracking-[0.5em] uppercase">寻之不得，或入他途</p>
              </div>
            ) : (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-6 relative z-10">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] font-black text-[#D4AF65] mt-1 uppercase tracking-[0.2em]">{starText}</span>
        </div>
        <div
      ...
      <div className="flex-1 relative z-10">
         <p className="text-[11px] font-medium text-[#5C4033]/70 leading-relaxed italic line-clamp-3 mb-6">
           {card.prologue ? `“${card.prologue}”` : (card.description ? `“${card.description.slice(0, 40)}...”` : '圣哲之言，待启。')}
         </p>
      </div>
      ...
      {!unlocked && (
        <div className="absolute inset-0 bg-[#FDFBF7]/60 backdrop-blur-[2px] flex items-center justify-center transition-all">
          <div className="bg-[#1A1A1A] text-white px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.3em] shadow-2xl scale-90 group-hover:scale-100 transition-transform">
            待启：名士等级 {getCardUnlockLevel(card)}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-5 duration-1000">
      <div className="flex items-center gap-6">
        <div
          className="w-20 h-20 bg-[#1A1A1A] flex items-center justify-center text-4xl font-black italic shadow-2xl relative"
        >
        ...
        <div>
          ...
          <div className="flex items-center gap-4 mt-2">
            <span className="text-xs font-black text-[#D4AF65] tracking-[0.3em]">{starText}</span>
      ...
      <div className="space-y-12">
        {card.prologue && (
          <div className="relative px-10 py-2">
            ...
            <p className="text-lg font-black text-[#1A1A1A] italic leading-loose serif opacity-80">
              “{card.prologue}”
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-6">
              ...
              {card.hp !== undefined && (
                <div className="p-8 rounded-[2.5rem] bg-white border-2 border-[#8D2F2F]/10 shadow-[0_15px_40px_rgba(141,47,47,0.05)] flex flex-col items-center group overflow-hidden relative">
                  ...
                  <span className="text-[11px] font-black text-[#8D2F2F] uppercase tracking-[0.3em] mb-2 relative z-10">基：学识</span>
        ...
        {card.tags && card.tags.length > 0 && (
          <div className="flex flex-wrap gap-3">
            {card.tags.map((tag, i) => (
              <span key={i} className="px-5 py-2 rounded-full border border-[#1A1A1A]/10 text-[10px] font-black text-[#5C4033]/60 uppercase tracking-[0.2em] bg-white/50">
                # {tag}
              </span>
            ))}
          </div>
